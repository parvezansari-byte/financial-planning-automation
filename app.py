...
# =====================================================
# FREEDOM | FINAL SINGLE FILE APP.PY
# Mutual Fund Product & Research Analyst Platform
# =====================================================
...
def advisor_note(title, lines):
    ...
    st.markdown('<div class="section-header">Advisor Product Notes</div>', unsafe_allow_html=True)
    st.markdown(f"**{title}**")
    for line in lines:
        st.write(f"• {line}")
        ...
        if resp.status_code == 200:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                if "schemeCode" in master_df.columns and "schemeName" in master_df.columns:
                    return master_df.rename(columns={"schemeCode": "Scheme Code", "schemeName": "Scheme Name"}), "LIVE"
...
if st.session_state.page == "sip":
    ...
    with t1:
        ...
        corpus = 0
        total_invested = 0
        current_sip_amt = monthly_sip
        for y in range(1, years + 1):
            for _ in range(12):
                corpus = corpus * (1 + sip_return / 12) + current_sip_amt
                total_invested += current_sip_amt
            current_sip_amt *= (1 + step_up)
...
if st.session_state.page == "swp":
    ...
    years_survive = int(initial_corpus / yearly_withdrawal) if yearly_withdrawal > 0 else 0
    kpi_row([
        ("Corpus", fmt(initial_corpus)),
        ("Yearly SWP", fmt(yearly_withdrawal)),
        ("SWR", f"{swr:.2f}%"),
        ("Indicative Years", str(years_survive))
    ])
...
if st.session_state.page == "retirement":
    ...
    addl_sip = monthly_sip_required(required_corpus, expected_return, years_to_ret)
    kpi_row([
        ("Expense at Retirement", fmt(expense_at_ret)),
        ("Target Corpus", fmt(required_corpus)),
        ("Required SIP", fmt(addl_sip))
    ])
...
if st.session_state.page == "fund_suggestion":
    ...
    if import_master:
        ...
        if not imported_master_df.empty:
            preview_cols = [col for col in ["Scheme Code", "Scheme Name"] if col in imported_master_df.columns]
    ...
    if nav_source == "AMFI/MFAPI Live Fetch" and refresh_live:
        ...
        st.success("Live NAV refresh attempted. Static fallback retained where API data was unavailable.")
    ...
    if len(display_df) == 0:
        ...
        advisor_note("Fund Research Notes", [
            "This module is positioned for product selection and client suitability.",
            "Live NAV fetch includes safe fallback to demo NAV when API is unavailable.",
            "Use AMC + category filters for faster distributor research workflow."
        ])
